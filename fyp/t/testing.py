...
for i in range(num_frames):
    ...
    if len(upscale_bbox) > 0:
        ...
        if rshX>0 and rkneeX>0 and lshX>0 and lkneeX>0: 
            shwidth=Distance(rshX,rshY,lshX,lshY)
            umidX = (rshX+lshX)/2
            umidY = (rshY+lshY)/2
            lmidX = (rfootX+lfootX)/2
            lmidY = (rfootY+lfootY)/2
            angle=(math.atan2(umidY-lmidY, umidX-lmidX) * 180 / 3.14)-270
            body_height=Distance(umidX,umidY,lmidX,lmidY)
            s_img = cv2.imread("fyp/t/images/didasha/dida"+shirt_ch+".png", -1)
            h=s_img.shape[0]
            w=s_img.shape[1]
        ...
        if lshX>0 and lelbX>0:
            ...
            s_img = cv2.imread("fyp/t/images/didasha/lsh"+shirt_ch+".png", -1)
        ...
        if rshX>0 and relbX>0:
            ...
            s_img = cv2.imread("fyp/t/images/didasha/rsh"+shirt_ch+".png", -1)
        ...
        if glass_enabled:
            if right_eyeX>0 and left_eyeX>0 and right_earX>0 and left_earX>0:
                ...
                if height>0 and width>0:
                    ...
                    height = int(s_img.shape[0]*width/s_img.shape[1]*size)
                    start_point = (glassx,glassy) 
                    end_point = (glassx-220, glassy-220) 
                    color = (255, 0, 0) 
                    thickness = 2

                    image = cv2.rectangle(s_img, start_point, end_point, color, thickness) 
